		...
		class Enlighten_Welcome {
			...
			public function __construct() {
				...
				$this->tab_sections = array(
					...
					'more_wp_stuff' => __('More WordPress Stuff', 'enlighten'),
				);
			...
			public function enlighten_activation_admin_notice() {
				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					...
					<div class="notice notice-success is-dismissible">
						<p><?php /* translators: %1$s : theme name, %2$s : welcome page link */ printf( wp_kses(__( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'enlighten' ), array( 'a' => array( 'href' => array() ) ) ), esc_html($this->theme_name), esc_url(admin_url( 'themes.php?page=enlighten-welcome' ))  ); ?></p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=enlighten-welcome' )); ?>"><?php esc_html_e( 'Lets Get Started', 'enlighten' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function enlighten_welcome_screen() {
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo esc_url(admin_url('themes.php?page=enlighten-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function enlighten_plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'enlighten' ) );
				...
				if (! wp_verify_nonce( $nonce, 'enlighten_plugin_installer_nonce' ))
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'enlighten') );
			...
			public function enlighten_plugin_activation_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'enlighten' ) );
				...
				if (! wp_verify_nonce( $nonce, 'enlighten_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'enlighten' ) );
