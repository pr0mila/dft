...
<table class="form-table">
    ...
    <tr>
        <td><em class="f13"><?php /* translators: %s : theme option page link */ echo sprintf(__('You can set up the sidebar content <a href="%s" target="_blank">here</a> in Sidebar tab','enlighten'), esc_url(admin_url('/themes.php?page=theme_options'))); ?></em></td>
    </tr>
</table>
...
function enlighten_save_sidebar_layout( $post_id ) { 
    ...
    if ( !isset( $_POST[ 'enlighten_sidebar_layout_nonce' ] ) || !wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[ 'enlighten_sidebar_layout_nonce' ] ) ), basename( __FILE__ ) ) )
        return;
...
foreach ($enlighten_sidebar_layout as $field) {  
        ...
        $new = isset( $_POST['enlighten_sidebar_layout'] ) ? sanitize_text_field( wp_unslash( $_POST['enlighten_sidebar_layout'] ) ) : '';
        if ($new && $new != $old) {  
